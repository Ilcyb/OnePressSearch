...
def get_sort_set_from_redis(redis_conn, set_name, num, queue):
    try:
        priority_set_list = redis_conn.zrevrange(
            set_name, 0, num - 1, withscores=True)
        if rem_sort_set_from_redis(redis_conn, set_name, num):
            for priority_set in [
                (i[1], i[0].decode()) for i in priority_set_list
            ]:  # 调换url和priority的顺序
...
def put_element_into_sort_set(redis_conn, set_name, element_value,
                              element_priority):
    try:
    ...
    def update(self, progress=0):
        ...
        with self.lock:
            ...
            sys.stdout.write("\r%s:[%s] %d/%d" %
                             (self.title, hashes + spaces,
                              self._current_progress, self.total))
...
def save_progress(progress_file, crawled_queue, cleaned_queue,
    ...
    try:
        redis_conn = _Redis(
            ...
            db=kwargs['redis_db'],
            password=kwargs['redis_pwd']).getRedisConn()

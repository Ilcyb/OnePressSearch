...
def search(keyword):
    ...
    for word in keyword_list:
        if word.encode() in all_word_set:
            url_list = [i.decode() for i in redis_conn.lrange(word + '_url_list', 0, -1)]
            tfidf_list = [float(i.decode()) for i in redis_conn.lrange(word + '_tfidf_list', 0, -1)]
            for url in url_list:
    ...
    return jsonify(dict(lengths=len(sorted_url_list), urls=sorted_url_list, keyword=keyword)), 200

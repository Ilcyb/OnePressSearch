...
def get_redis_conn(file_path):
    ...
    try:
        ...
        _redis = _Redis(redis_config.get('HOST', '127.0.0.1'), redis_config.get('PORT', 6379),
                            redis_config.get('DB', 0), redis_config.get('PASSWORD', None))
        redis_conn = _redis.getRedisConn()
        return redis_conn
    except KeyError:
        return _Redis('127.0.0.1', 6379, 0).getRedisConn()
    except redis.exceptions.ConnectionError as e:
        print('无法连接配置为',_redis.getRedisConf(),'的redis服务器，请检查redis服务器是否开启，连接参数是否填写正确')
